...
export function registerAdminRoutes(app: FastifyInstance, options: RegisterAdminOptions): void {
  ...
  app.get("/api/admin/errors", async () => ({ errors: await options.data?.recentErrors() ?? [] }));
  app.get<{ Params: { sessionId: string }; Querystring: { format?: string } }>("/api/admin/sessions/:sessionId/export", async (request, reply) => {
  ...
  for (const action of ["start", "idle", "skip", "restart"] as const) {
    app.post(`/api/admin/${action}`, async (_request, reply) => {
      const engine = options.engine();
