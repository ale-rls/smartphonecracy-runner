...
describe("MovementRecorder", () => {
  ...
  it("starts a named recording on session join, with samples relative to join", () => {
    const { recorder, started } = makeRecorder();
    recorder.join("p1", "Ada", "session-1", 1_000);
  ...
  it("accumulates elapsed-ms relative to the recording's own start, not wall-clock or phase time", () => {
    ...
    recorder.join("p1", "Ada", "session-1", 5_000);
  ...
  it("flushes a batch once the sample-count threshold is hit", () => {
    ...
    recorder.join("p1", "Ada", "session-1", 0);
  ...
  it("clamps out-of-range coordinates and drops non-finite samples", () => {
    ...
    expect(batches).toEqual([{
      ...
      batchIndex: 0,
      recordedAt: 20,
      samples: [{ t: 10, x: 0, y: 1 }, { t: 20, x: 0.3, y: 0.3 }],
    }]);
  });

  it("persists and finalizes a zero-sample participant recording", () => {
    const { recorder, started, batches, finalized } = makeRecorder();
    recorder.join("p1", "Ada", "session-1", 1_000);
    recorder.leave("p1", 5_000);
    expect(started).toEqual([expect.objectContaining({ participantId: "p1", participantName: "Ada", startedAt: 1_000 })]);
    expect(batches).toEqual([]);
    expect(finalized).toEqual([{ recordingId: "session-1:p1:1000", endedAt: 5_000, status: "abandoned", sampleCount: 0 }]);
  });
  ...
  it("finalizeSession() flushes and finalizes every open recording as completed, no-op otherwise", () => {
    const { recorder, finalized } = makeRecorder({ sampleThreshold: 1_000 });
    recorder.join("p1", "Ada", "session-1", 0);
    recorder.join("p2", "Lin", "session-1", 0);
    recorder.recordSample("p1", 0.1, 0.1, 0);
    // p2 never sends a sample, but still remains part of the session ledger.
  ...
  it("a full drop-then-reconnect within the same session produces two independent recordings", () => {
    ...
    recorder.join("p1", "Ada", "session-1", 3_000);
  ...
  it("starts its own timer on start() and flushes via the interval; stop() halts it", () => {
    ...
    recorder.start();
    recorder.start();
    recorder.join("p1", "Ada", "session-1", Date.now());
