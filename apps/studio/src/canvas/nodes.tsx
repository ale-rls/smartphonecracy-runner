...
export function PhaseNode({ data }: NodeProps) {
  ...
  return <div className={`studio-node ${value.kind}`}><Handle type="target" position={Position.Left} /><strong>{value.label}</strong><small>{value.kind}</small>
...
export function EndNode() { return <div className="studio-node marker"><Handle type="target" position={Position.Left} /><strong>End</strong><small>compiles to idle</small></div>; }
