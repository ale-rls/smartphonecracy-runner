...
export function mediaRows(project: StudioProject): MediaRow[] {
  return project.manifest.files.map((file) => ({
    ...file,
    references: project.scenario.phases.filter((phase) => (phase.kind === "video" || phase.kind === "video-position-question") && phase.src === file.src).map((phase) => phase.id),
  }));
}

export function distinctReferencedBytes(project: StudioProject): number {
  const referenced = new Set(project.scenario.phases.flatMap((phase) => phase.kind === "video" || phase.kind === "video-position-question" ? [phase.src] : []));
  ...
  const bytesFor = (start: string) => {
    ...
    while (queue.length) {
      ...
      if (item.kind === "video") { media.add(item.src); queue.push(item.next); }
        ...
        if (item.kind === "video-position-question") media.add(item.src);
