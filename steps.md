...
- verify: `pnpm --filter server exec vitest run src/engine/phase-engine.test.ts` → PASS (5 tests); `pnpm --filter server exec vitest run src/admission/admission.test.ts src/engine/phase-engine.test.ts` → PASS (13 tests); `pnpm -r typecheck` → PASS (7 workspaces), 2026-07-12. Full `pnpm --filter server test` reached 16/17 tests; the pre-existing real-localhost WebSocket test is sandbox-blocked by `listen EPERM: operation not permitted 127.0.0.1`.
- reviewer: claude
...
- notes: MediaStore (injectable caches/fetch/digest): boot sync keyed by content hash, byte+sha256 verify per download, stale-entry eviction, endless capped-backoff retry (never ready with missing media); Blob URLs memoized per src, retainOnly() revokes outside the active set; useMedia hook gates UI on ready + visible retry state; public/sw.js is app-shell only (never intercepts /media/), cache-first hashed assets, network-first HTML. Next-video preload wired but waits on STEP-026 (id→src map) — only the active video gets a Blob URL until then. FIXES 2026-07-12: cached entries now re-hashed at boot (corruption test added); showVideo race purges stray Blob URLs via retainOnly on stale resolve; pending video re-resolves when sync reaches ready; SW narrowed to /assets/ + navigations only. Back to review.
  ...
  Verification by codex: `pnpm --filter @smartphonecracy/display typecheck` PASS; `pnpm --filter @smartphonecracy/display test` PASS (18 tests, 2 files), 2026-07-12. Status remains `review` pending the requested fixes.
