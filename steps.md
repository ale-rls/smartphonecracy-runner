...
- reviewer: claude
- notes: Ready for Fable review. Added HMAC-SHA256, timing-safe, installation/room-scoped rotating join grants with 120-second expiry; signed 2-hour installation-scoped participant leases; in-memory participant registry with stable identity/color assignment, same-lease socket replacement, 30-participant connected cap, lease reconnect at capacity, and expiry pruning; process-local per-IP join throttling; and WebSocket handling that runs every raw payload through `parseClientMessage`, emits identity/rejection/pong messages, and closes malformed or duplicate joins. Added config secret wiring and server boundary integration. Fable should specifically review token parsing, expiry boundaries, scope checks, and replay/capacity semantics. CHANGES REQUESTED (fable review 2026-07-12): (1) requestIp trusts x-forwarded-for unconditionally — honor XFF only behind a config-gated trustProxy flag, else socket.remoteAddress; (2) cap is enforced on connectedCount — lease churn can accumulate unexpired leases that all reconnect past 30; add disconnect-grace removal in the registry (plan §6 heartbeat-timeout removal) or count held-unexpired leases; (3) InMemoryIpRateLimiter buckets never prune — sweep expired windows to bound memory against IP spray. Crypto core approved as-is (kind-bound HMAC, timingSafeEqual, expiry both directions, scoping).
